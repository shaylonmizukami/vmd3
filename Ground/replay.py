...
def main(filepath, mode):
    ...
    def vmd3_thread():
        ...
        for index in header_indices:
            ...
            payloadLen = int.from_bytes(data[index+4:index+8], byteorder='little')
            payload = data[index+8:index+8+payloadLen]
            ...
            print(f'Frame Number: {frameNum}, Payload Size: {len(payload)}')
            frameNum += 1
            ...
            # SAVE UNFILTERED SLOW TIME SAMPLES
            averaged_chirps = np.mean(fft_angle, axis=1)
            all_slow_time_samples.append(averaged_chirps[0])
            time_interval, data_all = processTimeDomain(all_slow_time_samples)
            x_fft, y_fft = processFreqDomain(data_all)
            # window.setTimeDomain(time_interval, data_all[:len(time_interval)])
            # window.setFreqDomain(x_fft, y_fft)

            # BEAMFORMING
            nRx = fft_range.shape[2]
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="ALiSM Replay Script for .bin Files")

    parser.add_argument('--filepath', type=str, help='File path for .bin file')
    parser.add_argument("--mode", type=int, default=6, help="Mode of the VMD3 radar data (0-8). Default: 6")

    args = parser.parse_args()

    main(filepath=args.filepath, mode=args.mode)
