...
Usage:
    python analyze_scan.py --session data/scan_2026-05-18
    python analyze_scan.py --session data/scan_2026-05-18 --show
"""
...
MAX_FRAMES_USED = 30     # Cap on how many frames to average per capture

# Range window around the auto-detected plate range
RANGE_WINDOW_HALF_M = 0.15   # ±15 cm window
...
def compute_range_angle_heatmap(payloads, vmd3):
    ...
    Returns a numpy array shaped (n_range_bins, n_angle_bins), with magnitude in linear units. Returns None if no usable frames were found.
    """
...
def estimate_plate_range(heatmap):
    """
    Find the strongest peak in the heatmap, excluding the near-range leakage zone (first ~30 cm), and return its range in meters.
    """
