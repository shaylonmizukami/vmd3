...
# =====================================================================
BINARY_FILEPATH = (
    "/home/shaylon/repos/ALiSM_Python_copy/vmd3_test/data/radc-rfft/2026-06-02/"
    "plate_6m_1p6mm_0p2hz_rset1.bin"
)
CONFIG_MODE = "2D"   # "2D" or "3D"
MAX_RANGE = 10       # Meters
TRIM_FRAMES = 1        # Number of startup frames to discard
MANUAL_TARGET_BIN = 80   # set to an int (e.g. 80) to force a bin; None = auto-detect
...
def decode_rfft_2d(frame):
    ...
    Datasheet layout: channel-major, then range bin, then sweep.
      RX1[ bin0:(sweep0..63), bin1:(sweep0..63), ... ], RX2[...], RX3, RX4
    Output shape: (128 range_bins, 64 chirps, 4 channels) to match RADC convention.
    """
    if len(frame) != 131072:
        raise ValueError(f'INVALID RFFT FRAME LENGTH FOR 2D MODE: {len(frame)}')
    raw = np.frombuffer(frame, dtype='<i2')          # 65536 int16
    raw = raw.reshape(4, 128, 64, 2)                 # (ch, range_bin, sweep, [I,Q])
    i_values = raw[:, :, :, 1].astype(np.float64)
    q_values = raw[:, :, :, 0].astype(np.float64)    # Q-first, matching your RADC convention
...
def main():
    ...
    n_frames = len(frames)
    slow_time_signal = np.zeros(n_frames, dtype=np.complex128)
    ...
    for i, frame in enumerate(frames):
        ...
        # On the first frame, lock onto the target's range bin, or use manually selected frame if set
        if target_range_bin is None:
            if MANUAL_TARGET_BIN is not None:
                target_range_bin = MANUAL_TARGET_BIN
                src = 'manual'
            else:
                leakage_bins_to_skip = 10
                search_region = range_profile[leakage_bins_to_skip:]
                idx = int(np.argmax(search_region))
                target_range_bin = idx + leakage_bins_to_skip + 1
                src = 'auto'
            target_distance = (target_range_bin - 1) * MAX_RANGE / fft_range.shape[0]
            print(f'\n>> Target locked at range bin {target_range_bin} '
                  f'({target_distance:.2f} m) [{src}]')

        # Extract complex value at target range bin,
        # averaged across all chirps and RX channels (one complex sample per frame)
        complex_at_target = np.mean(
            fft_range[target_range_bin - 1, :, :]  # -1 for 0-indexing
        )
    ...
    LAMBDA = 3e8 / 61.06e9   # wavelength at center frequency, ~4.87 mm
    ...
    plt.figure()
    plt.plot(t, I_component, linewidth=1.2, label='I (real)')
    plt.plot(t, Q_component, linewidth=1.2, label='Q (imag)')
    plt.xlabel('Time (s)')
    plt.ylabel('Amplitude')
    plt.title('Slow-time signal: I and Q components at target range bin')
    plt.legend()
    plt.grid(True)
    ...
    # ---- RADC vs RFFT validation (range profile from frame 0 of each) ----
    radc_frames = get_frames(BINARY_FILEPATH, b'RADC', CONFIG_MODE)
    rfft_frames = get_frames(BINARY_FILEPATH, b'RFFT', CONFIG_MODE)

    if radc_frames and rfft_frames:
        radc_cube = decode_radc_2d(radc_frames[TRIM_FRAMES])
        my_rfft   = np.fft.fft(radc_cube, axis=0)         # your own range FFT
        fpga_rfft = decode_rfft_2d(rfft_frames[TRIM_FRAMES])

        my_prof   = np.mean(np.abs(my_rfft),   axis=(1, 2))
        fpga_prof = np.mean(np.abs(fpga_rfft), axis=(1, 2))

        nbins = my_prof.shape[0]
        rng = np.arange(nbins) * MAX_RANGE / nbins

        plt.figure()
        # normalize each to its own max so shapes can be compared directly
        plt.plot(rng, my_prof / my_prof.max(), lw=1.4, label='Computed range FFT (from RADC)')
        plt.plot(rng, fpga_prof / fpga_prof.max(), lw=1.4, ls='--', label='FPGA RFFT')
        plt.xlabel('Range (m)'); plt.ylabel('Normalized magnitude')
        plt.title('RADC-computed range FFT vs FPGA RFFT')
        plt.legend(); plt.grid(True)
    else:
        print(f'[validation] RADC frames: {len(radc_frames)}, '
              f'RFFT frames: {len(rfft_frames)} — need both to compare.')
