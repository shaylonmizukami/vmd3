...
class RadarSignals(QObject):
    heatmap_ready   = pyqtSignal(object)          # range_angle array
    bf_time_ready   = pyqtSignal(object, object)  # x_td, y_td
    bf_freq_ready   = pyqtSignal(object, object)  # x_fd, y_fd
    peak_ready      = pyqtSignal(float, float)    # angle_deg, range_m
...
running = True   # headless-mode stop flag
# ─────────────────────────────────────────────────────────────────
...
def handle_sigint(signum, frame):
    ...
    """
    global vmd3_thread_running, _sigint_count, running
    running = False
    _sigint_count += 1
    if _sigint_count == 1:
...
class MainWindow(QMainWindow):
    def __init__(self, output_dir, csv_path):
        ...
        # Position selector (Option 4: dropdown + free-text override)
        ...
        # Thread-safe bridge from radar thread → GUI
        self.signals = RadarSignals()
        self.signals.heatmap_ready.connect(self.range_angle.setData)
        self.signals.bf_time_ready.connect(self.bf_time_domain.set_plot)
        self.signals.bf_freq_ready.connect(self.bf_freq_domain.set_plot)
        self.signals.peak_ready.connect(self.setPeakLabels)
...
def radar_thread_fn(window):
    ...
    while vmd3_thread_running:
        ...
        window.signals.heatmap_ready.emit(range_angle)
        ...
        window.signals.peak_ready.emit(peak_angle_deg, peak_range_m)
        ...
        window.signals.bf_time_ready.emit(x_td, y_td[:len(x_td)])

        x_fd, y_fd = process_freq_domain(y_td)
        window.signals.bf_freq_ready.emit(x_fd, y_fd)
...
# Map stream names → RdotConfig for the selector
STREAM_MAP = {
...
def run_headless(vmd3, output_dir, csv_path, scan_pos, streams,
                 n_frames_target, notes):
    """CLI-per-position acquisition: record frames for one Galil position."""
    import socket

    # Which UDP headers to accept, based on selected streams
    header_map = {'radc': b'RADC', 'rfft': b'RFFT', 'done': b'DONE'}
    accepted = tuple(header_map[s] for s in streams)

    # Build filename from position
    if scan_pos < 0:
        pos_str = f'neg{abs(scan_pos)}cm'
    elif scan_pos > 0:
        pos_str = f'pos{scan_pos}cm'
        ...
        pos_str = '0cm'
    filename = f'scan_{pos_str}.bin'
    filepath = os.path.join(output_dir, 'bins', filename)

    out = open(filepath, 'wb')
    start_time = datetime.now()
    n_written = 0
    bytes_written = 0
    counts = {h: 0 for h in accepted}

    print(f'[measure] Recording {streams} at {scan_pos} cm -> {filename}')
    if n_frames_target > 0:
        print(f'[measure] Target: {n_frames_target} frames. (Ctrl+C to stop early)')
    else:
        print(f'[measure] Recording until Ctrl+C.')
    ...
    try:
        while running:
            ...
            counts[frame[0:4]] = counts.get(frame[0:4], 0) + 1
            hdr = frame[0:4].decode('ascii', errors='replace')
            print(f'  frame {n_written:4d}  {hdr}  {bytes_written/1e6:.1f} MB')

            if n_frames_target > 0 and n_written >= n_frames_target:
                break
    finally:
        out.close()
    ...
    # Append CSV row (streams column added)
    ...
    breakdown = ', '.join(f'{h.decode()}:{c}' for h, c in counts.items())
    print(f'[measure] Saved {filename}: {n_written} frames ({breakdown}), '
          f'{bytes_written/1e6:.1f} MB, {duration:.1f} s')
    

def main(rset_config, session_name, gui, scan_pos, streams, n_frames_target, notes):
    ...
    if gui:
        ...
        # Headless mode: CLI-per-position acquisition
        if scan_pos is None:
            print('[measure] ERROR: --position is required in --no-gui mode.')
            vmd3.disconnect()
            sys.exit(1)
        selected = [STREAM_MAP[s] for s in streams]
        vmd3.set_output_config(selected)
        vmd3.sockUDP.settimeout(1.0)
        print(f'[measure] RSET={rset_config}, headless mode ready')

        run_headless(vmd3, output_dir, csv_path, scan_pos, streams,
                     n_frames_target, notes)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--no-gui', dest='gui', action='store_false',
        default=True, help='Headless CLI-per-position acquisition (no window)')
    parser.add_argument('--position', type=int, default=None,
        help='Galil position in cm for this capture (required with --no-gui)')
    parser.add_argument('--streams', type=str, default='radc,rfft,done',
        help='Comma-separated streams to save: radc,rfft,done (headless only)')
    parser.add_argument('--nFrames', type=int, default=5,
        help='Frames to record this position (0 = until Ctrl+C). Default 5.')
    parser.add_argument('--notes', type=str, default='',
        help='Optional notes logged to CSV')
    args = parser.parse_args()
    
    # Parse & validate streams
    streams = [s.strip().lower() for s in args.streams.split(',') if s.strip()]
    bad = [s for s in streams if s not in STREAM_MAP]
    if bad:
        print(f'[measure] ERROR: unknown stream(s): {bad}. '
              f'Valid: radc, rfft, done')
        sys.exit(1)

    main(args.rsetMode, args.sessionName, args.gui,
         args.position, streams, args.nFrames, args.notes)