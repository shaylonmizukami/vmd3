...
from vmd3lib.analysis import (
    range_profile, find_target_bins, bin_to_range, range_to_bin, range_fft,
)
from vmd3lib.displacement import (
    slow_time_signal, phase_to_displacement,
    motion_spectrum, displacement_spectrum, dominant_frequency,
    max_phase_step,
)
from vmd3lib.iq import remove_dc, arc_span_deg, revolutions
from vmd3lib.config import (
    MAX_RANGE_M, FS_SLOW, TARGET1_RANGE_M, TARGET2_RANGE_M, RANGE_WINDOW
)
...
def analyze_target(cubes, target_bin, angle_deg=None, channel=None):
    """
    ...
    sig_c = remove_dc(sig)
    step = max_phase_step(sig_c)
    print(f'  bin {target_bin} [{steer_label}]: max phase step {step:.2f} rad '
          f'({100 * step / np.pi:.0f}% of pi), '
          f'{revolutions(sig_c):.2f} revolutions')
    if step >= np.pi:
        print('  *** step exceeds pi — the displacement below is NOT valid ***')
...
def run_motion(cubes, n_targets, manual, angle=None, channel=None,
               search_window=None):
    ...
    results, spectra = [], []
    for b in bins:
        pp, fpk, f_d, m_d = analyze_target(cubes, b, angle_deg=angle,
                                           channel=channel)
...
def run_constellation(cubes, bins, channel=None, angle=None):
    """
...
if __name__ == '__main__':
    DEFAULT_FILE = '/home/shaylon/repos/ALiSM_Python_copy/vmd3-twotarget/data/radc/2026-08-04/01-t1-alone.bin'

    p = argparse.ArgumentParser(description='Analyze a VMD3 RADC capture.')
    p.add_argument('--file', default=DEFAULT_FILE, help=f'Path to the .bin file (default: {DEFAULT_FILE}).')
    p.add_argument('--mode', choices=['profile', 'motion', 'angle', 'constellation'], required=True)
    p.add_argument('--trim', type=int, default=1, help='Startup frames to drop (default: 1).')
    p.add_argument('--channel', type=int, default=None, choices=[0, 1, 2, 3],
                   help='Single RX channel (no array factor, no nulls). '
                        'Omit for a broadside sum — which nulls +/-30 deg.')
    p.add_argument('--window', type=str, default=None,
                   choices=['hann', 'rect'],
                   help='Override the config range window (A/B testing).')
    p.add_argument('--search', type=int, nargs=2, metavar=('LO', 'HI'),
                   default=None, help='Fence auto-detect to a bin range.')
    # profile
    p.add_argument('--avg', type=int, default=1, help='Frames to average for the profile (default: 1).')
    # motion
    p.add_argument('--targets', type=int, default=1, choices=[1, 2], help='Number of targets to analyze (motion mode).')
    # angle
    p.add_argument('--bin', type=int, default=None,
                   help='Range bin for angle mode / steered motion. '
                        'Angle mode requires it; motion uses it with --angle.')
    ...
    sel.add_argument('--auto', action='store_true', help='Auto-detect the strongest target bin(s).')
    sel.add_argument('--manual', type=int, nargs='+', metavar='BIN', help='Manually specify target bin(s), e.g. --manual 79 82.')
    args = p.parse_args()
    ...
    if args.mode == 'profile':
        ...
        bins = resolve_bins(cubes, args.targets, args.manual,
                            search_window=args.search)
        run_constellation(cubes, bins, channel=args.channel, angle=args.angle)
    else:   # motion
        run_motion(cubes, args.targets, args.manual, angle=args.angle,
                   channel=args.channel, search_window=args.search)
