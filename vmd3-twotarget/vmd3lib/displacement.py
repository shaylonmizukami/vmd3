...
def phase_step_stats(sig):
    """
    (max, 99th percentile) of the frame-to-frame phase step, in radians.
    Uses the unwrapped phase — np.angle alone is wrapped to (-pi, pi], so
    diffing it counts wrap crossings as ~2*pi steps rather than measuring
    real motion. Values are bounded by pi by construction; the closer they
    sit to pi, the closer the unwrap is to ambiguity.
    """
    d = np.abs(np.diff(np.unwrap(np.angle(sig))))
    return float(np.max(d)), float(np.percentile(d, 99))
...
def displacement_spectrum(displacement_mm, fs=FS_SLOW, n_pad=4096, window=SLOW_WINDOW, detrend=True):
    """
...
def dominant_frequency(freqs, mag, f_min=0.05):
    """
    ...
    """
    start = int(np.searchsorted(freqs, f_min))
...
def bandpass_filter(x, f_lo, f_hi, fs=FS_SLOW, order=4):
    """
    Zero-phase Butterworth band-pass for slow-time signals, in SOS form
    (the b/a form is numerically unstable at these low corner ratios).

    Use ~0.1-0.6 Hz to isolate breathing from postural drift before
    measuring peak-to-peak displacement.
    """
    x = np.asarray(x, dtype=float)
    nyq = fs / 2.0
    if not (0 < f_lo < f_hi < nyq):
        raise ValueError(f'Need 0 < {f_lo} < {f_hi} < Nyquist={nyq}.')
